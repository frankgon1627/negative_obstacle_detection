...
class NOORDetection(Node):
    ...
    def noor_detect(self):
        ...
        with ThreadPoolExecutor() as executor:
            futures = {
                executor.submit(self.process_scan_column, col_idx, point_array): col_idx
    ...
    def process_scan_column(self, column_idx: int, point_array: np.ndarray):
        feature_points: Set[Tuple[Tuple[float], float]] = set()
        ...
        for scan_line_idx in range(self.vertical_channels - 1, -1, -1):
            ...
            # exceeded threshold, mark this point as a feature point and calculate deviation
                ...
                risk: float = max(0.0, min(2.0, deviation / expected_threshold)) # TODO: make this better

                feature_points.add((tuple(last_valid_point), risk))
                feature_points.add((tuple(current_point), risk))
                last_valid_point = current_point
    ...
    def publish_feature_points(self, feature_points: Set[Tuple[Tuple[float], float]]):
        marker: Marker = Marker()
        marker.header.frame_id = "os_sensor" 
        marker.header.stamp = self.get_clock().now().to_msg()
        ...
        for point, risk in feature_points:
            x, y, z = point
            if not np.isnan(z):  # Ensure the point is valid
                new_point = Point32()
                new_point.x = float(x)
